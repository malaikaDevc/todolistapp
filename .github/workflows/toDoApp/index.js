...
function loadTodoList(userId) {
  ...
  if (user) {
    ...
    const toDoList = document.getElementById('toDo-list');
    toDoList.innerHTML = '';
    user.todos.forEach(({ taskText, description }) => {
      addTaskToDOM(taskText, description);
    });
...
updateButton.addEventListener('click', function () {
  ...
  if (user) {
    user.todos = user.todos.map(todo => 
      (todo.taskText === currentEditingTask.querySelector('label').innerText ? 
      { taskText: saveTask, description: saveDes } : todo)
    );
    localStorage.setItem('users', JSON.stringify(users));
  }
...
// Add tasks to the list of to-do tasks 
function addTask() {
  ...
  if (user) {
    // Store task and description as an object
    user.todos.push({ taskText, description: desText });
    localStorage.setItem('users', JSON.stringify(users));
...
// save tasks when page is refreshed 
function saveTasks() {
  const userId = parseInt(sessionStorage.getItem('loggedInUser'), 10);
  const user = users.find(user => user.id === userId);
  ...
  if (user) {
    ...
    user.todos = Array.from(document.querySelectorAll('#toDo-list label')).map((label, index) => {
      const des = document.querySelectorAll('#toDo-list p')[index].innerText;
      return { taskText: label.innerText, description: des };
    });
    localStorage.setItem('users', JSON.stringify(users));
  }
}
